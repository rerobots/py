...
class APIClient(object):  # pylint: disable=too-many-public-methods
    ...
    def create_access_rule(self, wdeployment_id, capability, to_user=None) -> str:
        ...
        if to_user is not None:
            body['user'] = to_user
        res = requests.post(
            self.__base_uri + '/deployment/{}/rule'.format(wdeployment_id),
            json=body,
            headers=self.__headers,
            verify=self.__verify_certs,
        )
    ...
    def get_instance_info(self, instance_id):
        ...
        """
        res = requests.get(
            self.__base_uri + '/instance/' + instance_id,
            headers=self.__headers,
            verify=self.__verify_certs,
        )
        if res.ok:
            ...
            if 'error_message' in payload:
                if payload['error_message'] == 'instance not found':
                    raise InstanceNotFound(
                        'no instance with identifier {}'.format(instance_id)
                    )
                raise Error(payload['error_message'])
            raise Error(payload)
        return payload
    ...
    def request_instance(
        ...
        if body:
            res = requests.post(
                self.__base_uri + '/new/' + type_or_wdeployment_id,
    ...
    def cancel_reservation(self, reservation_id):
        ...
        """
        res = requests.delete(
            self.__base_uri + '/reservation/' + reservation_id,
            headers=self.__headers,
            verify=self.__verify_certs,
        )
        if not res.ok:
    ...
    def get_firewall_rules(self, instance_id):
        ...
        """
        res = requests.get(
            self.__base_uri + '/firewall/' + instance_id,
            headers=self.__headers,
            verify=self.__verify_certs,
        )
        if not res.ok:
    ...
    def get_vpn_newclient(self, instance_id):
        ...
        res = requests.post(
            self.__base_uri + '/vpn/' + instance_id,
            headers=self.__headers,
            verify=self.__verify_certs,
        )
        if not res.ok:
    ...
    def activate_addon_vnc(self, instance_id):
        ...
        """
        res = requests.post(
            self.__base_uri + '/addon/vnc/' + instance_id,
            headers=self.__headers,
            verify=self.__verify_certs,
        )
        if not res.ok:
    ...
    def start_addon_vnc(self, instance_id):
        ...
        """
        res = requests.post(
            self.__base_uri + '/addon/vnc/' + instance_id + '/start',
            headers=self.__headers,
            verify=self.__verify_certs,
        )
        if not res.ok:
    ...
    def stop_addon_vnc(self, instance_id):
        ...
        """
        res = requests.post(
            self.__base_uri + '/addon/vnc/' + instance_id + '/stop',
            headers=self.__headers,
            verify=self.__verify_certs,
        )
        if not res.ok:
    ...
    def activate_addon_mistyproxy(self, instance_id):
        ...
        """
        res = requests.post(
            self.__base_uri + '/addon/mistyproxy/' + instance_id,
            headers=self.__headers,
            verify=self.__verify_certs,
        )
        if not res.ok:
    ...
    def get_snapshot_cam(
        ...
        if not res.ok:
            if res.status_code == 404:
                raise Error(
                    'instance {} not found, or cam add-on not active'.format(
                        instance_id
                    )
                )
            raise Error(res.text)
    ...
    def send_drive_command(self, instance_id: str, command: dict):
        ...
        res = requests.post(
            self.__base_uri + '/addon/drive/' + instance_id + '/tx',
            json=command,
            headers=self.__headers,
            verify=self.__verify_certs,
        )
    ...
    def submit_ci_job(self, pid, branch, ref):
        ...
        """
        res = requests.post(
            self.__base_uri + '/ci/project/{}/job'.format(pid),
